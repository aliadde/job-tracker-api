...
def test_get_current_usr_success(client, create_user):
    ...
    # Now send request get request to end point /current_user with Barear token set
    response = client.get("/current_user", headers={"Authorization": f"Bearer {access_token}"})
